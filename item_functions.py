...
def heal(*args, **kwargs):
    ...
    if entity.fighter.hp == entity.fighter.max_hp:
        ...
        results.append({'item_consumed': True, 'message': Message('Your wounds start to feel better!', libtcod.green)})
...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        results.append({'item_consumed': True, 'target': target, 'message': Message('A lighting bolt strikes the {0} with a loud thunder! The damage is {1}'.format(target.name, damage))})
        results.extend(target.fighter.take_damage(damage))
    else:
        results.append({'item_consumed': False, 'target': None, 'message': Message('No enemy is close enough to strike.', libtcod.red)})
...
def cast_fireball(*args, **kwargs):
    ...
    if not libtcod.map_is_in_fov(fov_map, target_x, target_y):
        results.append({'item_consumed': False, 'message': Message('You cannot target a tile outside your field of view.', libtcod.yellow)})
        return results
